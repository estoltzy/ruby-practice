...
class Todo
  def initialize(must_do, priority)
    @must_do = must_do
    @priority = priority
  end
...
class TodoList
  ...
  def print
    @todos.each do |object| puts "* " + object.text
    end
  end
end

wash_car = Todo.new("wash car", "high")
paint_fence = Todo.new("paint fence", "low")

ellie_todolist = TodoList.new
ellie_todolist.add(wash_car)
ellie_todolist.add(paint_fence)
ellie_todolist.print
